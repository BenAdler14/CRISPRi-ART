...
RUN="20240325_CRISPRiART_Requeue-725488425"
...
for dir_name in os.listdir("{project}/data/reads/{run}".format(project=PROJECT,run=RUN)) : 
    if any(sample_name in dir_name for sample_name in sample_list):
        new_dir_name = dir_name.split("-ds.")[0].split("_L1")[0] + "_L1"
...
rule all:
    input: 
        #trim reads
        #expand("{project}/data/tmp/{run}/trimming_completed.txt".format(project=PROJECT,run=RUN)),
        #count reads
...
#    output: 
#        completed = "{project}/data/tmp/{run}/{extended_sample_name}/trimming_completed.txt".format(project=PROJECT,run=RUN)
#    params:
...
rule count_reads:
    input: 
        expand("{project}/data/merged_reads/{run}/{extended_sample_name}_merged.fastq", project=PROJECT,run=RUN,extended_sample_name=extended_sample_list)
    output:
...
rule crispriart_fitness: 
    ...
    params: 
        ...
        base_dir_run_lib_gene = lambda wildcards: directory(expand("{project}/data/fitness/{run}/{lib_name}/gene_fitness".format(project=PROJECT,run=RUN,lib_name=wildcards.lib_name))),
        metadata = "{project}/data/metadata/{run}__metadata.tsv".format(project=PROJECT,run=RUN),
        guide_metadata = lambda wildcards: "{project}/oligos/{lib_name}_gRNA_metadata.csv".format(project=PROJECT,run=RUN,lib_name=wildcards.lib_name),
        lower_threshold = LT,
        upper_threshold = UT
    shell:
        """
        ...
        mkdir -p {params.base_dir_run_lib_gene}
        python crispriart_fitness.py \
            --counts {input.fast2q_counts} \
            --metadata {params.metadata} \
            --guide_metadata {params.guide_metadata} \
