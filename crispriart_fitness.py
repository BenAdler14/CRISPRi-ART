...
def get_args():
    ...
    parser.add_argument(
        ...
        default="",
        help="""
        Path to the input guide metadata file.
        """,
    )
...
def import_guide_metadata(path_guide_metadata) : 
    """
    Import guide metadata table.
    """
    df_guides = pd.read_csv(path_guide_metadata)
    return df_guides
...
def generate_fitness_outputs(path_out,sample_set): 
    dict_output = {}
    dict_output["counts_QC"] = os.path.join(path_out,"guide_fitness",sample_set+"__counts_QC.csv")
    dict_output["counts_norm"] = os.path.join(path_out,"guide_fitness",sample_set+"__counts_norm.tsv")
    dict_output["guide_fitness"] = os.path.join(path_out,"guide_fitness",sample_set+"__fitness_guides.tsv")
    dict_output["gene_fitness"] = os.path.join(path_out,"gene_fitness",sample_set+"__fitness_genes.tsv")
    return dict_output

def fit_calc(df_counts, df_guides, cond_dict, lower_percentile, upper_percentile, path_out) : 
    """
    ...
    for sample_set in cond_dict.keys() :
        ...
        df_gene_fit = guide2genefit_mean_KS(df_guide_fit, df_guides, cond_dict[sample_set], dict_output["gene_fitness"])
...
def guide2genefit_mean_KS(df_guidefit, df_guides, cond_dict_sample_set, outpath) :
    """
    Determine guide fitness using a 2-sided KS statistic on guide fitness compared to the base condition. 

    NOTE: this function will have to be modified slightly if df_guides is modified. 
    """
    # generate cond_list from the cond_dict
    cond_list = []
    cond_list.extend(list(cond_dict_sample_set["base"].keys()))
    cond_list.extend(list(cond_dict_sample_set["sample"].keys()))
    
    # Filter df_guidefit to the condition(s) of interest
    df_guidefit = df_guidefit[df_guidefit['QC_pass']][['gRNA_name', 'QC_pass'] + cond_list]
    df_guidefit = df_guidefit.merge(df_guides[['gRNA_name','gene','gRNA_start','gRNA_end']], on='gRNA_name')

    #create KS distribution
    #retrieve baseline foldchange distribution. This assumes that the guides that do well aren't the majority of the population.
    baseline_dist = {}
    for foldchange in cond_list : 
        baseline_dist[foldchange] = df_guidefit[foldchange].to_list()
    
    # calculate mean guide fitness and KS per gene
    gene_fit = []
    KS_data = []
    gene_list = set(df_guidefit['gene'].tolist())
    for gene in gene_list:
        out_dict = {}
        out_dict['gene'] = gene

        #pull FC data
        df_guidefit_gene = df_guidefit[df_guidefit['gene'] == gene]
        
        #pull name of first oligo in series
        out_dict['gRNA_name'] = df_guidefit_gene.iloc[0]['gRNA_name']

        #create coordinate averages
        out_dict['gRNA_start'] = mean(df_guidefit_gene['gRNA_start'])
        out_dict['gRNA_end'] = mean(df_guidefit_gene['gRNA_end'])
        out_dict['num_guides'] = len(df_guidefit_gene['gRNA_name'])

        #calculate KS statistic
        for foldchange in cond_list:
            genefit_dist = df_guidefit_gene[foldchange].to_list()
            
            mean_result = mean(genefit_dist)
            out_dict['{}'.format(foldchange)] = mean_result
            
            KS_result = stats.ks_2samp(genefit_dist, baseline_dist[foldchange], alternative = "less")
            out_dict['{}_KSpvalue'.format(foldchange)] = KS_result[1]
            
        KS_data.append(out_dict)

    df_KS = pd.DataFrame(KS_data)
    df_KS = df_KS.sort_values(by='gRNA_start').reset_index(drop=True)
    df_KS.to_csv(outpath,sep='\t',index=False)
...
def write_checkpoint(sample_set_list, path_out) : 
    with open(os.path.join(path_out,"fitness_checkpoint.txt"),"w") as f : 
...
def main():
    ...
    df_guide_metadata = import_guide_metadata(args.guide_metadata)

    print("Calculating fitness...")
    sample_set_list = fit_calc(df_counts, df_guide_metadata, cond_dict, float(args.lower), float(args.upper), args.output)
